					...
					buttons: {
						confirm: function () {
							...
							//$('.preview-images-zone').attr('id', 'overlay').html('<div style="text-align:center;color: black;font-size: 25px;margin-top: 60px;">Deleting...</div>');
							 var dataString = "requestType=Trusted&pid="+<?php echo CAREER_PAGE_ID; ?>+"&deleteId="+deleteImage;
							$.ajax({
								type: "POST",
    ...
    if (window.File && window.FileList && window.FileReader) {
        ...
        for (let i = 0; i < files.length; i++) {
		 ...
		 if(isValidExt){
			...
			formdata.append("pid", <?php echo CAREER_PAGE_ID; ?>);
...
function load_trusted_data(){
	var dataString = "requestType=Trusted&pid="+<?php echo CAREER_PAGE_ID; ?>;
