<div class="page-wrapper">
	<div class="content container-fluid">
		...
		// Form Open
		echo form_open('admin/team_list', $form_attribute, $hidden);
		?>
		<div class="row filter-row">
			...
			<div class="ml-3">
				...
				<button type="button" class="btn btn-danger" onclick="javascript:window.location.href='<?= base_url('admin/team_list') ?>';"> Clear</button>
			</div>
		</div>
		<?php
		// Form Close
							...
							?>
								<tr role="row" class="<?= $class ?>">
									...
									<td class="lightgallery">
										<?php
										$pagefilename = 'uploads/team_images/' . $testimonialList->picture;
										...
										if (file_exists($pagefilename) && !empty($testimonialList->picture)) {
											$page_file = '../uploads/team_images/' . $testimonialList->picture;
											$page_original_file = '../uploads/team_images/' . $testimonialList->picture;
										}
										?>
									...
									<td>
										<div class="dropdown action-label">
											...
											<div class="dropdown-menu">
												<a class="dropdown-item" href="<?= base_url('admin/team_status?do=pending&team_id=' . $testimonialList->id) ?>"><i class="fa fa-dot-circle-o text-warning"></i> Pending</a>
												<a class="dropdown-item" href="<?= base_url('admin/team_status?do=publish&team_id=' . $testimonialList->id) ?>"><i class="fa fa-dot-circle-o text-success"></i> Publish</a>
												<a class="dropdown-item" href="<?= base_url('admin/team_status?do=draft&team_id=' . $testimonialList->id) ?>"><i class="fa fa-dot-circle-o text-danger"></i> Draft</a>
											</div>
											<?php } ?>
										</div>
									...
									<td class="text-right">
									<?php
										if (getUserCan('team_module', 'access_write') || getUserCan('team_module', 'access_delete')) {
										?>
		...
		<div class="modal-content modal-lg" style="width:100%">
			...
			<div class="modal-body">
				<div class="m-b-30">
					...
					// Form Open
					echo form_open_multipart('admin/add_team', $form_attribute, $hidden);
					?>
					<div class="row">
						<div class="col-sm-6">
	...
	<div class="modal-dialog">
		<div class="modal-content modal-md">
			...
			<div class="modal-body card-box">
				...
				//Form Open
				echo form_open('admin/delete_team', $form_attribute, $hidden);
				?>
				<p>Do you want to delete the team now with his related table data? This cannot be undone.</p>
...
<script>
	...
	function getEditData(testimonial_id) {
		...
		validator.resetForm();
		$("#ckeditor-msg").html("");
		var dataString = "request=edit_team_data&team_id=" + testimonial_id;
		//alert(dataString);
		//return false;
		jQuery.ajax({
			type: "POST",
			...
			success: function(res) {
				...
				if (res.dataContent) {
					if (res.dataContent != '') {
						...
						$("form[name=edit-testimonial] #linkedin").val(res.dataContent.linkedin);
						...
						if (res.dataContent.picture != '') {
							img_src = '../uploads/team_images/' + res.dataContent.picture;
						}
	...
	function getAllPagesData(testimonial_id) {
		var dataString = "request=get-all-pages&type=TM&id=" + testimonial_id;
		//alert(dataString);
		//return false;
		jQuery.ajax({
			type: "POST",
	...
	function getAllCategoriesData(testimonial_id) {
		var dataString = "request=get-all-categories&type=TM&id=" + testimonial_id;
